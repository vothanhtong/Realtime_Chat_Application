...
export function SignupForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div className={cn("w-full max-w-sm mx-auto relative", className)} {...props}>
      ...
      <button
        onClick={toggleTheme}
        className="absolute -top-16 right-0 p-3 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 text-white hover:bg-white/20 transition-all dark:bg-slate-900/50 dark:border-slate-700/50"
      ...
      <div className="rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl p-8 dark:bg-slate-900/50 dark:border-slate-700/50">
        ...
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-white mb-2 dark:text-slate-100">Create Account</h1>
          <p className="text-white/60 text-sm dark:text-slate-400">Join us today, it's free!</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Input
                placeholder="Họ"
                className={cn(
                  "h-11 rounded-2xl bg-white/10 border-white/20 text-white placeholder:text-white/40",
                  "focus:bg-white/15 focus:border-white/40 focus-visible:ring-0 focus-visible:ring-offset-0",
                  errors.lastName && "border-red-400/60"
                )}
                {...register("lastName")}
              />
              {errors.lastName && <p className="text-red-300 text-xs mt-1 ml-1">{errors.lastName.message}</p>}
            </div>
            <div>
              <Input
                placeholder="Tên"
                className={cn(
                  "h-11 rounded-2xl bg-white/10 border-white/20 text-white placeholder:text-white/40",
                  "focus:bg-white/15 focus:border-white/40 focus-visible:ring-0 focus-visible:ring-offset-0",
                  errors.firstName && "border-red-400/60"
                )}
                {...register("firstName")}
              />
              {errors.firstName && <p className="text-red-300 text-xs mt-1 ml-1">{errors.firstName.message}</p>}
            </div>
          </div>
          ...
          <div>
            <Input
              ...
              {...register("username")}
            />
            {errors.username && <p className="text-red-300 text-xs mt-1 ml-1">{errors.username.message}</p>}
          </div>
          ...
          <div>
            <div className="relative">
              <Input
                ...
                className={cn(
                  "h-11 rounded-2xl bg-white/10 border-white/20 text-white placeholder:text-white/40 pr-12",
                  "focus:bg-white/15 focus:border-white/40 focus-visible:ring-0 focus-visible:ring-offset-0",
          ...
          <Button
            ...
            className="w-full h-12 rounded-2xl font-bold text-white text-base border-0 bg-gradient-to-r from-blue-500 to-cyan-400 hover:from-blue-600 hover:to-cyan-500 hover:shadow-lg hover:shadow-cyan-500/25 transition-all duration-300 disabled:opacity-60 mt-2"
          >
